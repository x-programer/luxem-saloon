...
import { Bell, X, Calendar, Check, Trash2, Loader2 } from "lucide-react";
...
import Link from "next/link"; // For navigation to bookings
...
export function BookingNotificationBell() {
    ...
    const [dismissedIds, setDismissedIds] = useState<string[]>([]); // Local state for dismissed notifications
    ...
    useEffect(() => {
        ...
        console.log("🔔 NotificationBell: Listening for pending bookings...");
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            const newBookings = snapshot.docs.map(doc => ({
                id: doc.id,
                ...doc.data(),
                createdAt: doc.data().createdAt?.toDate ? doc.data().createdAt.toDate() : new Date() // Fallback
            }));

            // Client-side sort to avoid composite index requirements for now (status + date)
                        ...
                        <div className="max-h-[60vh] overflow-y-auto p-2 space-y-1">
                            {loading ? (
                                ...
                                visibleBookings.map((booking) => (
                                    <Link href="/dashboard/bookings" key={booking.id} onClick={() => setIsOpen(false)}>
                                        <div className="group relative p-3 rounded-xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100 cursor-pointer">
                                            ...
                                            <button
                                                ...
                                                className="absolute top-2 right-2 p-1.5 rounded-full text-slate-300 hover:text-red-500 hover:bg-white transition-colors opacity-0 group-hover:opacity-100"
                                                title="Dismiss"
                                            >
